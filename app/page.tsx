  ...
  return <SiteShell title="Experiencia completa">
    ...
    <section className="home-official-section home-access" aria-labelledby="home-access-title">
      ...
      <div className="home-access-grid">
        {quickServices.map(({image,title,copy,cta,href,featured,logo}) => <article className={featured ? "featured card-clickable" : "card-clickable"} key={title}><Link className="card-cover-link" href={href} aria-label={title} />{logo ? <Image className="service-logo" src={image} alt="" width={140} height={56} unoptimized /> : <span><Image src={image} alt="" width={28} height={28} unoptimized /></span>}<h3>{title}</h3><p>{copy}</p><span className="card-cover-cta">{cta} <ArrowRight /></span></article>)}
        <article className="provider-portal card-clickable"><Link className="card-cover-link" href="https://humana.med.ec/portal-prestador/" aria-label="Portal del Prestador" /><span className="home-service-icon"><Building2 /></span><div><h3>Portal del Prestador</h3><p>El portal donde los prestadores de salud administran su convenio, y donde las instituciones puedan iniciar su proceso de inclusión.</p></div><span className="card-cover-cta provider-portal-cta">Acceso <ArrowRight /></span></article>
      </div>
