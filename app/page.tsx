...
export default function Home() {
  return <SiteShell title="Experiencia completa">
    ...
    <section className="home-official-section home-access" aria-labelledby="home-access-title">
      ...
      <div className="home-access-grid">{quickServices.map(({icon:Icon,title,copy,cta,href,featured}) => <article className={featured ? "featured" : ""} key={title}><span><Icon /></span><h3>{title}</h3><p>{copy}</p><Link href={href}>{cta} <ArrowRight /></Link></article>)}</div>
    ...
    <section className="home-official-section home-about" aria-labelledby="home-about-title">
      ...
      <div className="home-about-copy"><p><strong>Humana S.A.</strong> es una compañía de salud y medicina prepagada que forma parte del grupo más importante en prestaciones médicas en el Ecuador: <strong>Conclina C.A.</strong>, al que también pertenece el Hospital Metropolitano, Fundación Metrofraternidad, y Metrored.</p><p>Nuestra trayectoria de <b>29 años</b> en el mercado nos ha permitido cuidar del tesoro más preciado: <strong>La salud y el bienestar de miles de familias ecuatorianas.</strong> Actualmente contamos con más de <b>200.000 afiliados</b> a nivel nacional.</p><p>Humana pone al servicio de sus afiliados la <strong>red de prestadores médicos más representativa, seguros y confiables del país</strong>, garantizando una atención médica de calidad:</p><p>Somos una empresa líder en <strong>planes de medicina prepagada</strong> que cuida el <strong>bienestar de personas y empresas</strong> ecuatorianas desde <b>1994</b>. Estamos aquí para respaldarte con una amplia gama de <strong>coberturas y red de clínicas, hospitales, médicos, centros médicos, laboratorios y farmacias</strong> en todo el país. Nos adaptamos a tus necesidades y te ofrecemos <strong>acompañamiento 24/7</strong> para guiarte en el uso de tu plan cuando lo necesites.</p><Link href="/conocenos">Conoce más sobre Humana <ArrowRight /></Link></div>
    </section>
  </SiteShell>;
}
